...
func getArgs(opts *Opts) ([]string, error) {
    ...
    if operation == Add{
        if len(opts.Args) != 3 {
             return nil, fmt.Errorf("add requires 2 arguments, only recieve %v", len(opts.Args) - 1)
        }
    ...
    if operation == Remove{
        if len(opts.Args) != 1 {
             return nil, fmt.Errorf("Remove requires 1 arguments, only recieve %v", len(opts.Args) - 1 )
        }
        return opts.Args[1:], nil
    }

    if len(opts.Args) > 1{
         return nil, fmt.Errorf("Remove requires 0 or 1 arguments, only recieve %v", len(opts.Args) - 1)
    }
